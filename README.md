...
独立的 Python / Flask 成员管理工具，当前版本 **0.4.0**。主要使用方式为 Mac 桌面应用，与直播监控、录制和上传程序分开运行。
...
当前 52 项自动测试覆盖成员添加／修改、冲突恢复、访问保护、SSH 操作、日志读取边界及加密迁移。已验证打包后的桌面界面、真实 Oracle 读取及三台服务器日志读取；新增和修改流程使用隔离演示数据验证，没有通过测试写入真实生产成员。
